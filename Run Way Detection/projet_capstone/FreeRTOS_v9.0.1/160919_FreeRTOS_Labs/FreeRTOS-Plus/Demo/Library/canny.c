    ...
    bitmap_info_header_t* bitmapInfoHeader)
{
    ...
    size_t pad, count = 0;
    unsigned char c;
    pad = (size_t)(4 * ceil((uint64_t)bitmapInfoHeader->bitspp * bitmapInfoHeader->width / 32.) - bitmapInfoHeader->width);
...
uint8_t save_bmp(const char* filename, const bitmap_info_header_t* bmp_ih,
    const pixel_t* data)
{
    ...
    size_t pad = (size_t)(4 * ceil((uint64_t)bmp_ih->bitspp * bmp_ih->width / 32.) - bmp_ih->width);
    unsigned char c;
...
static void convolution(const pixel_t* in, pixel_t* out, const float* kernel,
 ...
 */
static void gaussian_filter(const pixel_t* in, pixel_t* out,
    const int nx, const int ny, const float sigma)
{
    const int n = 2 * (int)(2 * sigma) + 3;
    const float mean = (float)floor(n / 2.0);
    //float kernel[n * n]; // variable length array
    float* kernel = calloc(n * n, sizeof(float));
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
            kernel[c] = (float)(exp(-0.5 * (pow((i - mean) / sigma, 2.0) +
